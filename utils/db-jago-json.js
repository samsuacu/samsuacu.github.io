async function getUsers() {
  ...
  getUsers().then((data) => {
    ...
    table += `
      <div class="card">
        <h5 class="card-header">Script Copy Rek</h5>
        <div class="card-body">
          <p class="card-text">
    `;
    ...
    data.forEach((user) => {
      ...
      table += `
        <pre>function ${user.username.substring(0, user.username.length-2)}() {
  navigator.clipboard.writeText("${user.rek}");
}</pre>
      `;
    });
